...
class ReconciliationReceiptHash:
    """Immutable observer-only hash result for a reconciliation receipt."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.algorithm != "sha256":
            ...
            raise ReconciliationReceiptHashingError(
        ...
        if self.execution_requested is not False:
            raise ValueError(
                "ReconciliationReceiptHash must remain observer-only."
            )

        if self.side_effects_permitted is not False:
            raise ValueError(
                "ReconciliationReceiptHash must not permit side effects."
            )
