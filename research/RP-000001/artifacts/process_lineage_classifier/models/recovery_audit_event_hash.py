...
class RecoveryAuditEventHash:
    """Immutable observer-only hash result for a recovery audit event."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.algorithm != "sha256":
            ...
            raise RecoveryAuditEventHashingError(
        ...
        if self.execution_requested is not False:
            raise ValueError(
                "RecoveryAuditEventHash must remain observer-only."
            )

        if self.side_effects_permitted is not False:
            raise ValueError(
                "RecoveryAuditEventHash must not permit side effects."
            )
