...
from services.restart_classifier import (
...
def test_classifier_accepts_warm_restart() -> None:
    previous = make_event(
        event_id="EV-001",
        sequence_number=1,
    ...
    current = make_event(
        ...
        evidence_ids=("EVD-RESTORE-001",),
        snapshot_id="SNAPSHOT-001",
    )

    result = RestartClassifier().classify(previous, current)

    assert result.event_type is EventType.RESTART
    ...
    assert result.transition_status is TransitionStatus.RESTARTED
    assert result.operational_status is OperationalStatus.PASS
    assert result.confidence is ConfidenceLevel.HIGH
    assert "RS-001" in result.applied_rules


def test_classifier_accepts_cold_restart() -> None:
    previous = make_event(
        event_id="EV-001",
        sequence_number=1,
        event_type=EventType.START,
    ...
    assert result.runtime_continuity is ContinuityStatus.TERMINATED
    assert result.execution_continuity is ContinuityStatus.TERMINATED
    assert result.state_continuity is ContinuityStatus.INTERRUPTED
    assert result.state_lineage is LineageStatus.NEW_ROOT
    assert result.binding_status is BindingStatus.BOUND
    assert result.transition_status is TransitionStatus.RESTARTED
    assert result.operational_status is OperationalStatus.PASS
    assert result.confidence is ConfidenceLevel.HIGH
    assert "RS-002" in result.applied_rules


def test_classifier_explains_warm_restart() -> None:
    previous = make_event(
        event_id="EV-001",
        sequence_number=1,
        event_type=EventType.START,
...
def test_classifier_rejects_wrong_current_event_type() -> None:
    ...
    with pytest.raises(RestartClassificationError, match="RESTART"):
        RestartClassifier().classify(previous, current)
...
def test_classifier_rejects_unchanged_runtime_identity() -> None:
    ...
    with pytest.raises(RestartClassificationError, match="(?i)runtime"):
        RestartClassifier().classify(previous, current)
...
def test_classifier_rejects_unchanged_execution_identity() -> None:
    ...
    with pytest.raises(RestartClassificationError, match="(?i)execution"):
        RestartClassifier().classify(previous, current)
...
def test_classifier_rejects_changed_service_identity() -> None:
    ...
    with pytest.raises(RestartClassificationError, match="(?i)service"):
        RestartClassifier().classify(previous, current)
...
def test_classifier_rejects_missing_parent_link() -> None:
    ...
    with pytest.raises(RestartClassificationError, match="(?i)parent"):
        RestartClassifier().classify(previous, current)


def test_classifier_rejects_partial_restore_evidence() -> None:
    previous = make_event(
        event_id="EV-001",
        sequence_number=1,
        event_type=EventType.START,
    ...
    with pytest.raises(RestartClassificationError, match="(?i)evidence"):
        RestartClassifier().classify(previous, current)
